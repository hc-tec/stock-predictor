...
def  storeCommentInf(comment):#存储评论
    db = pymysql.connect(host="127.0.0.1",user="root",password="root",db="text_analysis",port=3306)
    cur=db.cursor()
...
def selectCommentOrderByDate(share_code,method):#查询评论信息
    ...
    cur = db.cursor()
    if  method==0:#按照日期升序
        sql = 'SELECT * FROM TB_COMMENT WHERE SHARE_CODE LIKE %s ORDER BY DATE '
    else:#按照日期降序
        sql='SELECT * FROM TB_COMMENT WHERE SHARE_CODE LIKE %s ORDER  BY DATE DESC '
    cur.execute(sql,(f'%{share_code}'))
    db.commit()
    cur.close()
    return cur.fetchall()
